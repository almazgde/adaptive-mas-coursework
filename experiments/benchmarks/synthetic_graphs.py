...
class SyntheticGraphFactory:
    ...
    @staticmethod
    def wide_sparse(node_count: int = 16, seed: int = 0) -> DAG:
        """Low dependency density with high parallelism."""
        rng = random.Random(seed)
        dag = DAG()
        for index in range(node_count):
            dag.add_node(SyntheticGraphFactory._node(index, "research", rng))
        for index in range(0, max(0, node_count - 4), 4):
            if index + 4 < node_count:
                dag.add_edge(Edge(f"n{index}", f"n{index + 4}"))
        return dag

    @staticmethod
    def deep_dependency(node_count: int = 16, seed: int = 0) -> DAG:
        """Long critical path with mostly sequential dependencies."""
        rng = random.Random(seed)
        dag = DAG()
        for index in range(node_count):
    ...
    @staticmethod
    def layered(node_count: int = 16, seed: int = 0) -> DAG:
        """Mixed topology with layered fan-out and fan-in."""
        rng = random.Random(seed)
        dag = DAG()
        for index in range(node_count):
            task = "synthesize" if index >= node_count - 2 else "research"
            dag.add_node(SyntheticGraphFactory._node(index, task, rng))

        layers = SyntheticGraphFactory._layers(node_count, [3, 5, 5, 3])
    ...
    @staticmethod
    def centralized_coordinator(node_count: int = 16, seed: int = 0) -> DAG:
        """Hierarchical graph with a central coordinator and aggregation."""
        rng = random.Random(seed)
        dag = DAG()
        for index in range(node_count):
            if index == 0:
        ...
        for index in range(1, node_count - 1):
            ...
            elif index + 1 < node_count - 1 and index % 3 == 1:
