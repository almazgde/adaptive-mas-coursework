...
from adaptive_mas.execution import ExecutionManager
...
def run_scenario(name: str, dag: DAG, topology_type: TopologyType):
    ...
    executor = ExecutionManager.create_executor(dag, topology_type)
    ...
    with open(f"results/scenario_{name.replace(' ', '_')}_{topology_type.value}_trace.json", "w", encoding="utf-8") as handle:
        json.dump(trace.__dict__, handle, indent=2)
...
def main():
    ...
    for name, dag, topology in scenarios:
        run_scenario(name, dag, topology)
